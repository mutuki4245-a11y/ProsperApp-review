...
営業中端末は店舗1台で、会計準備中の伝票を別端末から変更しない運用を前提にする。`store.issue_checkout_statement` が返す `print_data` は、物理印刷開始前に同一ブラウザの会計伝票用 `localStorage` キューへ保存する。同じ端末での印刷失敗・再印刷は保存済みデータを使うため、通常は追加RPCを呼ばない。会計準備中は自由に再印刷でき、紙面に `再印刷` などの表記を付けず、印刷履歴も保存しない。キュー項目は `slip_id` を識別子にし、`印刷待ち`、`印刷中`、`印刷失敗`、`印刷済み`、`店員完了` を表す。未完了状態の間は会計確定を無効にし、印刷成功または店員の完了操作で完了状態にする。完了状態を含む記録は会計確定RPCの成功応答時に削除し、会計確定に失敗した場合は残して再利用できるようにする。
...
| 会計伝票出力と会計準備中 | 後続仕様 | `store.issue_checkout_statement`、`store.get_checkout_statement_print_data`、`store.release_checkout_ready` を追加し、`store_slips.status` に `checkout_ready` を追加する。`store.issue_checkout_statement` は退店時刻固定、`checkout_ready` 化、未退店客行補完、印刷用データ返却までを1RPCで完結させる。返却された印刷用データは物理印刷前に営業中端末の会計伝票用localStorageへ保存し、同一端末の再印刷では追加RPCを呼ばず再利用する。印刷待ち、印刷中、印刷失敗、印刷済み、店員完了はDB状態にせず、この端末内記録で表す。未完了状態の間は会計確定を無効にし、印刷成功または店員の「印刷完了として進む」操作で完了状態にする。端末記録は会計確定の成功応答時に削除し、記録がない `checkout_ready` は印刷完了とみなさず復旧用RPCで会計伝票を再取得する。`store.confirm_checkout` は `checkout_ready` 伝票だけを対象にし、`open` からの直接確定を禁止する。会計確定自体は `checkout_ready` から `checked_out` へ遷移するDB操作である。`store.confirm_checkout` から `p_closed_at` と `p_confirmed_snapshot` は外し、会計伝票出力時に固定済みの `store_slips.closed_at` と支払入力を使う。`store_slip_customers.left_at_source` を追加し、個別退店登録は `manual`、会計伝票出力補完は `accounting_slip` として記録する。出力前には対象伝票の最新保存状態を1RPCで取得し、営業中一覧の表示状態だけを根拠にしない。差分があった場合も再クリックは要求せず、画面表示と出力対象を最新状態へ寄せる。物理印刷失敗ではDB状態を戻さず、再印刷または解除で戻す。`store.release_checkout_ready` は `checkout_ready` だけを対象にし、`checked_out` は既存の会計取消へ分ける。再印刷は専用スナップショットを作らず、会計準備中でロックされた現在DB状態から復旧時だけ再生成する。解除RPCでは `store_slips.closed_at = null` に戻し、`accounting_slip` 由来の退店時刻だけを未確定に戻し、個別退店登録済みの退店時刻は残す。`store.add_order_lines` など会計内容を変えるRPCは `checkout_ready` を対象外にする。営業中一覧の定期更新では印刷用JSONを返さない。 |
