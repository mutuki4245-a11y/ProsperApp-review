...
アプリ側では、店舗一覧、店舗コンテキスト、卓、キャストマスタ候補、商品候補、商品管理カタログ、キャスト管理一覧、指名バック設定、現在営業日、当日出勤キャスト候補を `IMemoryCache` の対象として扱う。RPC失敗や設定未完了の結果はキャッシュしない。商品/カテゴリ保存、商品削除、商品並び順保存、キャスト登録/削除、指名バック設定保存、営業日開始、営業日締めなどの破棄契機は `HANDOFF.md` の重要方針に従う。
