...
アプリ側では、店舗一覧、店舗コンテキスト、卓、キャストマスタ候補、商品候補、商品管理カタログ、キャスト管理一覧、現在営業日を `IMemoryCache` の対象として扱う。RPC失敗や設定未完了の結果はキャッシュしない。商品/カテゴリ保存、商品削除、商品並び順保存、キャスト登録/削除、営業日開始、営業日締めなどの破棄契機は `HANDOFF.md` の重要方針に従う。
...
| 指名種別別キャストバック | 後続仕様 | 指名価格の保存と会計加算は実装済み。指名種別に応じたキャストバック計算は未実装で、保存先と集計方法を別途定義する。 |
