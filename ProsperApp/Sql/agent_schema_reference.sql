...
create table if not exists public.store_item_master (
    ...
    item_type text not null default 'standard', -- standard / karaoke
...
-- store_nomination_back_master: per-store cast back amount by nomination type.
...
--   get_store_nomination_back_master(p_department_id bigint)
--     returns one row for each nomination type: nomination, in_store, companion.
--   save_store_nomination_back_master(p_department_id bigint, p_settings jsonb)
--     upserts store_nomination_back_master rows. p_settings uses { nomination_type, back_unit_amount, sort_order, is_active }.
...
--     total includes product subtotal, 20% service tax on product subtotal, nomination_price, and active adjustment charge lines.
...
--     )
--     returns slip_id
--     initial nominations also create store_slip_cast_backs snapshots from the current nomination back master.
