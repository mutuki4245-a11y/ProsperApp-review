...
as $$
    with target as (
        select
            ...
            s.department_id,
            s.table_id,
            s.table_code_snapshot,
            s.table_name_snapshot,
            c.checkout_id,
            c.checkout_at,
            c.subtotal_amount,
            c.service_charge_amount,
    ...
    select
        ...
        t.business_date,
        t.table_id,
        case when t.table_code_snapshot is not null then t.table_code_snapshot else tm.table_code end as table_code,
        case when t.table_code_snapshot is not null then t.table_name_snapshot else tm.table_name end as table_name,
        t.checkout_id,
        t.checkout_at,
        t.subtotal_amount,
        t.service_charge_amount,
