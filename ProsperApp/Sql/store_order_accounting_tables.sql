...
alter table public.store_staff_master
    add column if not exists employment_type text not null default 'employee';

alter table public.store_staff_master
    drop constraint if exists chk_store_staff_master_employment_type;

alter table public.store_staff_master
    add constraint chk_store_staff_master_employment_type
...
-- 締め作業で入力する、伝票に紐付かないドリンクバック調整です。
-- 出勤キャストは0円を含めて必須、出勤外キャストは明示された行だけを保存します。
...
-- 領収書の「前渡金: キャスト」を、営業日と出勤キャストへ監査可能な形で紐付けます。
-- 会計仕訳が正本であり、このテーブルは日報のキャスト別表示に使う補助台帳です。
create table if not exists public.store_business_day_cast_advances (
    ...
    memo text,
    created_at timestamp with time zone not null default now(),
    constraint chk_store_business_day_cast_advances_amount check (amount > 0),
    constraint uq_store_business_day_cast_advances_journal unique (journal_entry_id)
);

-- 領収書簡易入力で、店舗が従業員へ立替分を返済した営業日を記録します。
-- accounting.journal_entries.entry_date は領収書の取引日を保持し、日報はこの台帳の営業日で集計します。
...
create index if not exists idx_store_staff_attendance_day_status
    on public.store_staff_attendance(company_id, department_id, business_day_id, attendance_status);

create index if not exists idx_store_staff_attendance_department_day_status
    on public.store_staff_attendance(department_id, business_day_id, attendance_status, staff_id);

create index if not exists idx_store_staff_attendance_staff
    on public.store_staff_attendance(staff_id, business_date);
...
create index if not exists idx_store_business_day_cast_advances_day_cast
    on public.store_business_day_cast_advances(department_id, business_day_id, cast_id);

create index if not exists idx_store_business_day_cast_advances_business_day
    on public.store_business_day_cast_advances(business_day_id, cast_id);

create index if not exists idx_store_business_day_cast_advances_cast
    on public.store_business_day_cast_advances(cast_id);

create index if not exists idx_store_business_day_cast_advances_company
    on public.store_business_day_cast_advances(company_id);
...
alter table public.store_staff_master enable row level security;
...
alter table public.store_staff_attendance enable row level security;
...
alter table public.store_business_day_cast_advances enable row level security;
...
drop trigger if exists trg_store_staff_master_set_updated_at on public.store_staff_master;
create trigger trg_store_staff_master_set_updated_at
before update on public.store_staff_master
for each row execute function public.set_updated_at();
...
drop trigger if exists trg_store_staff_attendance_set_updated_at on public.store_staff_attendance;
create trigger trg_store_staff_attendance_set_updated_at
before update on public.store_staff_attendance
for each row execute function public.set_updated_at();
...
drop trigger if exists trg_store_business_day_drink_back_adjustments_set_updated_at on public.store_business_day_drink_back_adjustments;
