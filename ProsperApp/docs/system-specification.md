...
- `close_business_day` は readiness を再確認し、条件未達の場合は失敗する。
...
| キャッシュ | 期間 | 主な対象 |
| --- | --- | --- |
| マスタ | 10 分 | 店舗文脈、卓番、キャスト、商品、商品管理カタログ |
| 実行時 | 30 秒 | 現在営業日、出勤キャスト、注文対象、営業中関連状態 |
| Drive プレビュー | 10 分 | Drive metadata/media |
| 未処理領収書 | 30 秒 | 領収書一覧 |

管理者設定画面からアプリ内キャッシュを全削除できる。マスタ更新時は関連キャッシュを削除する。
...
- ドキュメントのみの変更は Azure App Service の直接デプロイ対象外である。
- C#、Razor、`wwwroot`、実行時設定を変更する場合は、ビルド確認後に `main` へ push し、必要に応じて publish profile による直接デプロイを行う。
- `.codex/prosper-web.PublishSettings`、一時 publish 出力、APIキー、ローカル設定はコミットしない。
- ローカル開発サーバーは、利用者から明示依頼がある場合だけ起動する。
