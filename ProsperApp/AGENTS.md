...
このリポジトリでは、GPT系モデルのサブエージェントを起動しない。

- サブエージェントを使う場合は、`.codex/agents/*.toml` で定義されたOpenRouter系の軽量エージェントだけを使う。
- `default`、`explorer`、`worker` など、親CodexのGPT系モデルを継承するサブエージェントへフォールバックしない。
- OpenRouterの認証未設定や401エラーでサブエージェント起動に失敗した場合も、GPT系サブエージェントで代替しない。
- その場合は、メインCodexで作業を続行するか、OpenRouter認証を直してから再実行する。
