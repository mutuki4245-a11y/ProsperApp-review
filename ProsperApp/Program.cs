...
var reviewMode = builder.Configuration.GetValue<bool>("Review:UseMockData");
var disableAuthentication = builder.Configuration.GetValue<bool>("Review:DisableAuthentication");

// Add services to the container.
builder.Services.AddRazorPages(options =>
{
    options.Conventions.AddPageRoute("/Attendance", "/Closing/Attendance");
});
...
if (reviewMode)
{
    builder.Services.AddSingleton<ISupabaseRpcClient, ReviewSupabaseRpcClient>();
    builder.Services.AddSingleton<IGoogleDriveAuthService, ReviewGoogleDriveAuthService>();
    builder.Services.AddSingleton<IDriveFileService, ReviewDriveFileService>();
}
else
{
...
app.MapStaticAssets();
app.MapDrivePreviewEndpoints(requireAuthorization: !disableAuthentication);
...
var logout = app.MapGet("/logout", async (HttpContext context, string? returnUrl) =>
    {
