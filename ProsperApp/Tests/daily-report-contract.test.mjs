...
const dailyReportPartial = await readFile(new URL('../Pages/Shared/_DailyReport.cshtml', import.meta.url), 'utf8');
...
assert.doesNotMatch(sql, /entry\.description/i, '摘要で存在しないjournal_entries.description列を参照しないこと');
...
assert.match(closingPage, /partial name="_DailyReport"/, '締め画面は共通日報Partialを利用すること');
assert.match(dailyReportPartial, /<th>勘定科目<\/th><th>摘要<\/th><th>金額<\/th>/, '支出内訳は摘要列を持つこと');
assert.match(dailyReportPartial, /data-report-visits data-min-rows="31"/, 'リスト表はA4余白を埋める既定行を持つこと');
assert.match(dailyReportPartial, /data-report-expenses data-min-rows="35"/, '支出内訳表はA4余白を埋める既定行を持つこと');
