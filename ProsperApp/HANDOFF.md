...
- `/Orders` は営業中端末の業務フロー外にある注文端末専用画面です。上部タブ、業務フローナビ、フッター、営業中への戻り導線を表示せず、管理者設定だけは画面右上の最小導線として残します。卓番選択、商品一覧、注文キューを3パネル横並びで表示します。各パネルはパネル内で縦スクロールします。複数卓の注文を同じキューに入れて一括登録でき、バック対象商品のキャスト候補には先頭に「なし」を出します。
...
- `/Orders`
  ...
  - 卓番選択、商品一覧、注文キューを3パネル横並びで表示し、それぞれのパネル内で縦スクロールします。
