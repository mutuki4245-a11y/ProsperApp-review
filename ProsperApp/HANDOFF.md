...
- 会計処理モーダルの決済方法入力は、0円会計で0円決済を許容し、決済方法の再描画だけで0円が合計額へ戻らないようにしています。現金決済額は、他の決済方法を併用した場合に会計額から非現金決済額を引いた残額へ同期します。
