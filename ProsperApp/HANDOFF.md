...
2026-08-03の全画面マスタ・キャッシュ統合では、旧 `store.get_business_home_bootstrap` を `store.get_store_bootstrap(p_department_id)` に置き換えました。このRPCは店舗コンテキスト、店舗一覧、卓番、キャスト、スタッフ、商品/カテゴリ、料金、指名バック、決済方法、現在営業日、出勤キャスト、営業中トップ初期snapshotを1回のEdge RPCで返します。C#側は一括応答を既存の型別マスタキャッシュへ展開し、マスタは更新時まで、営業日・出勤キャストは30秒保持します。マスタ更新時だけ関連キーとbootstrap payloadを削除し、営業日開始・締めでは静的マスタを削除しません。営業中トップの定期更新、focus復帰、online復帰は引き続き `store.get_business_day_snapshot` 経由です。キャッシュ一覧と全削除は `/Management` の最下部へ移し、通常の店舗設定利用者も実行できます。対象Supabaseには `store_master_bootstrap_rpc` migrationを適用し、`prosper-rpc` はversion 35でACTIVEです。新RPCの実応答、旧RPC削除、`security definer`、`search_path=public`、`public`、`anon`、`authenticated`、`service_role` の直接実行不可を確認済みです。

2026-08-04の締め条件変更では、領収書入力を営業日締めの必須条件から外しました。締め画面では未入力件数と任意の入力導線を独立取得して表示しますが、取得失敗や未入力件数は締めボタンへ影響させません。SQL定義コミットは `a3bea52` で、対象Supabaseには `exclude_receipts_from_closing_requirements` migrationを適用済みです。営業日52の未入力349件を残したまま、件数は返却され、`block_reasons` から領収書理由だけが外れること、`close_business_day` に領収書拒否が残っていないこと、両RPCの `anon` / `authenticated` 直接実行禁止を確認しました。ロールバック時はアプリを本変更前へ戻すだけではDBが戻らないため、`a3bea52^` の `01_business_day.sql` にある `get_business_day_closing_readiness` と `close_business_day` の2関数も再適用します。
...
- マスタを使う画面は、型別キャッシュ欠損時に `IStoreMasterBootstrapper` の `store.get_store_bootstrap` を1回だけ実行し、応答を全マスタ用キーへ展開します。マスタはTTLなしで更新時まで保持し、商品、卓、キャスト、スタッフ、料金、指名バックのアプリ内更新成功時に関連キーとbootstrap payloadを削除します。現在営業日と出勤キャストは30秒保持し、営業日開始・締め・勤怠保存時に更新します。領収書pending一覧など動的な締めデータはキャッシュ対象外です。キャッシュ一覧と一括削除は `/Management` の最下部にあり、複数App Serviceインスタンス間では共有しません。アプリ外のマスタ変更はキャッシュ削除またはアプリ再起動で反映します。RPC失敗や設定未完了の結果はキャッシュしません。
