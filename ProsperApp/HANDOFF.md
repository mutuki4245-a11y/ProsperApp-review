...
- `/Closing`
  ...
  - キャスト売上額調整パネルは専用ページ `/Closing/CastSalesAdjustment` へ遷移する導線として扱います。
...
- `/Closing/CastSalesAdjustment`
  - キャスト売上額調整の専用ページ。
  - 会計済みかつ指名キャストがいる伝票一覧を表示し、客名とキャストごとの売上分配額を一覧上で確認できるようにします。
  - 売上額調整は伝票行末のボタンから開くモーダルで行います。
