...
- 伝票追加モーダルは、時刻/卓番/メモ、客、指名の3カラム構成です。客・指名パネルは内容量だけを上から詰め、指名列を広く取って、種別・料金・キャスト選択を1行で扱います。説明文や補助文言、客・指名の行番号は出さず、同伴系の指名種別を選ぶと指名料金の初期値を3000円にします。
- 2026-07-21: 伝票追加モーダルの卓番パネルは、全高グリッドの余りを受け取らず、卓番リストの内容高とパディングだけで表示する。客・指名が多い場合も各リストの上限高だけをスクロールし、モーダル本文で追加分を扱う。
