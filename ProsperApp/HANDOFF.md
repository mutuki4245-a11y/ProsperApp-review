...
- 2026-07-18 決定記録: 会計・領収書・伝票詳細まわり（2026-07-19にソース実装済み）
  ...
  - 卓番マスタ/DB:
    - 卓番をカテゴリ分けできるDB変更を入れる。例: A卓 `A1` から `A6`、B卓 `B1` から `B6` をカテゴリ単位で表示/並び替えできるようにする。
    - `store_table_master` にカテゴリコード、カテゴリ名、カテゴリ表示順を持たせるか、卓番カテゴリマスタを分けるかを検討する。`store.get_tables`、`store.get_business_day_slips`、`store.get_order_entry_slips` の戻り値とUIグルーピングも合わせて更新する。
  - 時刻制約/RPC検証:
  ...
  - 伝票作成モーダル:
    - 伝票作成時に客人数を先に選択し、その人数に合わせて客行を自動生成する。
    - 指名入力は今のまま、必要な客行や指名対象を選べる構成を維持する。客人数変更時に既存入力を破棄するか、増減分だけ追加/削除するかはUI上で明確にする。
