...
- 未締めの前営業日が残っている場合は新しい営業日を作らず、ホームから締め作業へ誘導します。
...
- `/`
  - 店舗ホーム。
  - 営業中に操作する画面として扱い、伝票編集と勤怠入力を最優先に配置します。
  - 営業中、締め作業への導線と、営業中の伝票起こしを置きます。
  - 営業中の営業日がない場合は「最初の入力で営業日を自動作成」と表示し、明示的な営業日開始ボタンは置きません。
  - 酒代、領収書、締め前確認は補助操作として伝票編集・勤怠入力より下位に置きます。
  - キャスト情報と商品情報は、営業日前の必須作業ではなく、ホーム下段の随時管理から任意のタイミングで開く導線にします。

- `/Opening`
  - 旧URL互換用です。
  - 営業日は自動作成するため、GET/POSTともホームへリダイレクトします。

- `/Opening/Casts`
  - ホームの随時管理から開くキャスト情報確認/編集導線用。
...
- `/Closing`
  - 締め作業ホーム。
  - 酒代入力、勤怠入力、キャスト売上額調整、領収書入力、営業日締めへの導線を縦並びの独立パネルで表示します。
  - 酒代入力と勤怠入力は営業中にも入力できる随時入力で、締め作業では未入力の確認対象として扱います。
  - 営業日締めは、未会計伝票0、酒代入力済み、勤怠1名以上、退勤未入力0、未入力領収書0を満たした場合だけ実行できます。画面POSTと `close_business_day` RPCの両方で同じ条件を検証します。
