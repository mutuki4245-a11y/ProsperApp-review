...
- 店舗業務の大きな状態は `営業日開始 -> 営業中 -> 締め作業` を基本にします。ただし、画面内では「毎日必須」「必要なときだけ」「営業中に随時入力」「締め前確認」を区別し、時間順だけで必須度を誤解させないでください。
...
- `/`
  - 店舗ホーム。
  - 営業日開始、営業中、締め作業への導線と、営業中の伝票起こしを置きます。
  - キャスト情報と商品情報は、営業日開始前の必須作業ではなく、ホームの随時管理から任意のタイミングで開く導線にします。

- `/Opening`
  - 営業日開始専用画面。
  - 出勤登録、キャスト情報、商品情報はここに置かず、営業日の日付とメモだけで営業日を開始します。
